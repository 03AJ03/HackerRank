...
def get_json(url, params=None):
    ...
    if response.status_code in (401, 403):
        raise RuntimeError("HackerRank authentication failed. Refresh HACKERRANK_SESSION.")
...
def get_all_accepted_submissions():
    ...
    limit = 1000
    while True:
        data = get_json(f"{BASE_URL}/rest/contests/master/submissions/", {"offset": offset, "limit": limit})
...
def get_submission_details(challenge_slug, submission_id):
    data = get_json(f"{BASE_URL}/rest/contests/master/challenges/{challenge_slug}/submissions/{submission_id}")
...
def language_info(language):
    ...
    mapping = {
        "python": ("Python", ".py"), "python3": ("Python", ".py"), "pypy3": ("Python", ".py"),
        "sql": ("SQL", ".sql"), "mysql": ("SQL", ".sql"), "oracle": ("SQL", ".sql"), "db2": ("SQL", ".sql"),
        "java": ("Java", ".java"), "java8": ("Java", ".java"), "java15": ("Java", ".java"), "java17": ("Java", ".java"),
        "cpp": ("C++", ".cpp"), "c++": ("C++", ".cpp"), "c": ("C", ".c"),
        "csharp": ("C#", ".cs"), "c#": ("C#", ".cs"), "javascript": ("JavaScript", ".js"),
        "typescript": ("TypeScript", ".ts"), "go": ("Go", ".go"), "ruby": ("Ruby", ".rb"),
        "php": ("PHP", ".php"), "kotlin": ("Kotlin", ".kt"), "swift": ("Swift", ".swift"), "rust": ("Rust", ".rs"),
    }
...
def has_marker(marker):
    result = subprocess.run(["git", "log", "--all", "--format=%s", "--grep", marker], check=True, capture_output=True, text=True)
...
def sync_submission(submission):
    ...
    timestamp = parse_timestamp(details.get("created_at") or details.get("createdAt") or details.get("timestamp") or submission.get("created_at") or submission.get("timestamp"))
    ...
    if not file_path.exists() or file_path.read_text(encoding="utf-8").strip() != code.strip():
        ...
        commit_with_date(f"{marker} - {title}", timestamp, allow_empty=True)
